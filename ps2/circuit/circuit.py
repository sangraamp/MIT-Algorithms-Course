...
class PriorityQueue:
    ...
    def heapify(self,i):
        ...
        if(r<len(self.queue)):
            if(self.queue[r]<self.queue[smallest]):
                smallest=r
    ...
    def append(self, key):
        ...
        #self.min_index = None
        self.queue.append(key)
        keyIndex=len(self.queue)-1
        
        while(keyIndex!=0):
            if(self.queue[keyIndex]<self.queue[self.parent(keyIndex)]):
                self.queue[keyIndex],self.queue[self.parent(keyIndex)]=self.queue[self.parent(keyIndex)],self.queue[keyIndex]
                keyIndex=self.parent(keyIndex)
            else:
                break
