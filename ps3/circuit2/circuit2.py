...
class RangeIndex(object):
  ...
  def list(self, first_key, last_key): 
    return self.List(self.data,first_key,last_key)
  ...
  def lca(self, tree, l, h):
    node = tree.root
    while not(node == None or (l <= node.key and h >= node.key)):
      if l < node.key:
        node = node.left
      else:
        node = node.right
  ...
  def count(self, first_key, last_key):
    ...
    if self.data.find(first_key) is not None and self.data.find(last_key) is not None:
      return self.rank(last_key) - self.rank(first_key) + 1
    if self.data.find(first_key) is not None:
      return self.rank(last_key) - self.rank(first_key) + 1
